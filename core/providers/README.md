...
Shared behaviors: [shared/README.md](shared/README.md) - run `hawp providers sync` after editing.
...
4. Register the provider in `ActiveProviders` in `librarian/src/internal/domain/distribution/distribution.go`.
5. Add generated shared-behavior targets in `librarian/src/internal/domain/providersync/materialize.go` when the provider uses materialized rule files.
6. Update `distribution/sources/providers/README.md` and `distribution/generated/README.md`.
7. Run `hawp distribution sync`.
