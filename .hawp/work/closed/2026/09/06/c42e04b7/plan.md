...
Implemented 2026-09-06: Added "Path and Import Handling Protocol" section to `.github/copilot-instructions.md` with repo-root anchoring, import surgery rule, and staging proof requirements. Resolves the `model_commands.go` build-breaking import cleanup issue from session `47c793d6`. Verified `go run ./cmd/hawp check --no-update-check` passes all 3 validations (kit validate, work validate, links check). Plan closed via commit `92cb6ac`.
...
- `.github/copilot-instructions.md` updated with "Path and Import Handling Protocol" section (repo-root anchoring, import surgery rule, staging proof)
- `go run ./cmd/hawp check --no-update-check` passes all 3 validations (kit validate, work validate, links check)
- Resolves the `model_commands.go` build-breaking import cleanup from session `47c793d6`
...
The orphaned active copy was moved unchanged to [original intake](intake-original-archive.md)
on 2026-09-06. This preserves its contents without reopening the completed item.
