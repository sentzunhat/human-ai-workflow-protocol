...
Done. Both install/update source templates reject a symlinked `.hawp` root
before the existing nested-symlink scan and before any managed-tree writes.
`hawp distribution sync` regenerated all provider variants, generated scripts
pass `bash -n`, and distribution/repository validation passed.

## Next Step

- [x] Investigation recorded above
- [x] Plan written with options, recommendation, and verification scope
- [x] Backlog moved to `done`
