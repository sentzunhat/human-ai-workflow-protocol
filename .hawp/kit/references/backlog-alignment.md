...
Each work item lives in its own folder named by its ID:

| Layout | Path pattern | When to use |
|--------|-------------|-------------|
| Folder-per-item (default) | `active/{uuid}/plan.md` | All new work items |
| Flat file (legacy) | `active/{uuid}.md` | Legacy items; validator accepts both |

**Folder-per-item** is the current standard. It allows multiple supporting files
(`context.md`, `research.md`, evidence attachments) alongside the plan without
polluting the parent directory.

**IDs** are 8-character lowercase alphanumeric short IDs (e.g. `b7e2a4f9`) or
full UUIDs generated by `hawp work new "<title>"`. Legacy items may use `TASK-NNN`
or date-prefixed formats; the validator recognises all supported forms.
