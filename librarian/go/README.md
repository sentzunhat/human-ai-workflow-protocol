...
Go implementation lane of the librarian — the small native HAWP intelligence
tool. Lives alongside the current npm tooling in `librarian/`, which remains
the working implementation until its capabilities are ported here.
...
1. **Manual dispatch (recommended)** — GitHub Actions tab → this
   workflow → "Run workflow" → enter a `version` (e.g. `0.1.0`, no `v` or
   `librarian-go-` prefix). The workflow creates and pushes the
   `librarian-go-v<version>` tag itself; no local `git tag`/`git push`
   needed.
2. **Push a tag directly** — `git tag librarian-go-v0.1.0 && git push
   origin librarian-go-v0.1.0` also triggers the workflow.
