...
function main(): void {
  try {
    ...
    const generatedRoot = join(repoRoot, "distribution", "generated");

    for (const legacy of LEGACY_ROOT_GUIDES) {
      const legacyPath = join(generatedRoot, legacy);
      if (existsSync(legacyPath)) {
        legacyPresent.push(legacyPath);
      }
    }
