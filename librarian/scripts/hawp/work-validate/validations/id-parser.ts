...
export function extractIdFromFilename(filename: string): string | null {
  ...
  // Bare short-ID (exactly 8 alphanumeric chars, no slug): uuid folder names
  // like "b7e2a4f9" or "t5l7m9n1" used in the folder-per-item layout
  // (active/{id}/plan.md, closed/YYYY/MM/DD/{id}/plan.md).
  const bareShortId = filename.match(/^([0-9a-z]{8})$/i);
  if (bareShortId) {
    return bareShortId[1]!.toLowerCase();
  }
