...
const MAX_VERIFICATION_DETAIL_LINES = 20;
...
export const formatReport = (report: ValidationReport): string => {
  ...
  lines.push(`\n  Evidence-backed: ${vc.proven}/${vc.total}`);
  if (vc.unproven.length > 0) {
    lines.push(`  Explicitly unproven: ${vc.unproven.length}`);
    vc.unproven.slice(0, MAX_VERIFICATION_DETAIL_LINES).forEach((item) => {
      lines.push(`    ${item.id}: ${item.claim}`);
    });
    if (vc.unproven.length > MAX_VERIFICATION_DETAIL_LINES) {
      lines.push(
        `    ... ${vc.unproven.length - MAX_VERIFICATION_DETAIL_LINES} more explicitly unproven claim(s)`,
      );
    }
  }
  if (vc.ambiguous.length > 0) {
    lines.push(
      `  Ambiguous checklist claims (need Evidence: or explicit unproven label): ${vc.ambiguous.length}`,
    );
    vc.ambiguous.slice(0, MAX_VERIFICATION_DETAIL_LINES).forEach((item) => {
      lines.push(`    ${item.id}: ${item.claim}`);
    });
    if (vc.ambiguous.length > MAX_VERIFICATION_DETAIL_LINES) {
      lines.push(
        `    ... ${vc.ambiguous.length - MAX_VERIFICATION_DETAIL_LINES} more ambiguous checklist claim(s)`,
      );
    }
  }
  ...
  // Dead Links
  lines.push("5. DEAD LINKS");
  lines.push(
    "----------------------------------------------------------------------",
  );
  const dl = report.checks.deadLinks;
  lines.push(`\n  Scanned ${dl.scanned} file(s)`);
  if (dl.broken.length > 0) {
    lines.push(`  ✗ ${dl.broken.length} broken link(s):`);
    dl.broken.forEach((item) => {
      lines.push(`    ${item.file}: ${item.link}`);
    });
  } else {
    lines.push(`  ✓ No dead links`);
  }
