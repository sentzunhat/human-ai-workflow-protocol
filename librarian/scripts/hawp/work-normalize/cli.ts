...
export interface CLIOptions {
  ...
  exportResearchQueue?: string; // --export-research-queue <path>
...
export const parseArgs = (args: string[]): CLIOptions => {
  ...
  while (i < args.length) {
    ...
    if (arg === "--export-research-queue") {
      i++;
      const path = args[i];
      if (!path || path.startsWith("-")) {
        throw new CLIUsageError(
          "Error: --export-research-queue requires a file path",
        );
      }
      opts.exportResearchQueue = path;
      i++;
      continue;
    }
...
OPTIONS:
  ...
  --export-research-queue <path>
                         Export verification research queue JSON to file
...
EXAMPLES:
  ...
  # Export research queue for a follow-on agent
  hawp backlog upgrade --export-research-queue research-queue.json

NPM SCRIPT SHORTCUTS (from librarian/):
  npm run work:normalize:cli -- --dry-run --validate
  npm run work:normalize
...
For more information: https://github.com/sentzunhat/human-ai-workflow-protocol
`;
        ...
        `${JSON.stringify(
          {
            ...
            exportResearchQueue: opts.exportResearchQueue,
            output: opts.output,
