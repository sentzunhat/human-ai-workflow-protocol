...
test("runBacklogUpgradeScript writes output and exported plan files", async () => {
  await withTempRepo(async (repoRoot) => {
    ...
    const exportResearchQueuePath = join(repoRoot, "tmp", "research.json");
    ...
    const result = await runBacklogUpgradeScript({
      ...
      exportResearchQueue: exportResearchQueuePath,
      forceDirty: false,
      verbose: false,
      repoRoot,
    });
...
test("runBacklogUpgradeScript blocks apply mode on a dirty working tree", async () => {
  await withTempRepo(async (repoRoot) => {
    ...
    // Untracked seed files leave the working tree dirty.
    seedMinimalBacklog(repoRoot);
...
test("runBacklogUpgradeScript adds verification evidence follow-up items for ambiguous claims", async () => {
  await withTempRepo(async (repoRoot) => {
    execSync("git init -q", { cwd: repoRoot });
    seedMinimalBacklog(repoRoot);
    ...
    assert.ok(
      result.notices.some((notice) =>
        notice.includes("verification evidence follow-up item(s)"),
      ),
    );

    const updated = readFileSync(closedPath, "utf-8");
    assert.match(updated, /^### Evidence Follow-Up$/m);
    assert.match(updated, /Research evidence for: Smoke test completed/);
    assert.match(
      updated,
...
test("runBacklogUpgradeScript dry-run report includes verification research queue", async () => {
  await withTempRepo(async (repoRoot) => {
    seedMinimalBacklog(repoRoot);
    writeFileSync(
      join(
    ...
    assert.equal(result.exitCode, ExitCode.Success);
    assert.match(result.stdoutText, /Verification Research Queue/);
    assert.match(result.stdoutText, /TASK-702:\d+ Smoke test completed/);
  });
});

test("runBacklogUpgradeScript dry-run ignores evidence follow-up scaffolding as new drift", async () => {
  await withTempRepo(async (repoRoot) => {
    seedMinimalBacklog(repoRoot);
    writeFileSync(
      join(
      ...
      [
        "# Task: follow-up already scaffolded",
        "",
        "## Verification",
        "",
        ...
        "### Evidence Follow-Up",
        "",
        "- [ ] Research evidence for: Older ambiguous claim",
    ...
    assert.equal(result.exitCode, ExitCode.Success);
    assert.doesNotMatch(result.stdoutText, /TASK-703.*ambiguous verification/);
    assert.doesNotMatch(result.stdoutText, /TASK-703.*unproven verification/);
  });
});
