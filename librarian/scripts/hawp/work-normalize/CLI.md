# Work Normalize CLI Contract

This file documents the CLI adapter contract for work-normalize (invoked as `hawp backlog upgrade`).
...
Use `work:normalize:cli` for the raw CLI contract (`--dry-run`, `--apply`,
`--format`, `--export-plan`, `--export-research-queue`, etc). Use
`work:normalize` as the convenience shortcut for `--apply --validate`.
...
- `--export-research-queue <path>`: write the verification research queue JSON to file.
