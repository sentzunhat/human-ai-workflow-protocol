...
test("parseArgs parses default dry-run mode (parse-only)", async () => {
  const parsed = await parseArgs(["backlog", "upgrade"]);
...
test("parseArgs parses apply and optional flags (no execution)", async () => {
...
test("parseArgs parses help and version shortcuts", async () => {
  const helpParsed = await parseArgs(["--help"]);
  assert.ok(helpParsed);
  assert.equal(helpParsed.help, true);

  const versionParsed = await parseArgs(["-v"]);
