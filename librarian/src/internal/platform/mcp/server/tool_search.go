...
func toolSearchContext(query string, limit, maxTokens int, repoRoot string) rpcResponse {
	response, err := searchContextResponse(query, limit, maxTokens, repoRoot)
	if err != nil {
		return toolErr("search failed: " + err.Error())
	}
...
func searchContextResponse(query string, limit, maxTokens int, repoRoot string) (ContextSearchResponse, error) {
	...
	if len(results) == 0 {
		return ContextSearchResponse{Query: query, Budget: maxTokens}, nil
	}

	deduped, dropped := appcontext_layout1.ContentJaccardDedup(results, 0.70)
