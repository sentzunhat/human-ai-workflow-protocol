...
func TestWriteCodexTOMLCreatesFile(t *testing.T) {
	...
	expectedBin := hawpBinaryPath(repoRoot)
...
func TestWriteCodexTOMLUpgradesExistingRelativeBlock(t *testing.T) {
	...
	server := parsed["mcp_servers"].(map[string]any)["hawp"].(map[string]any)
...
func TestWriteCursorMCPJSONUsesNativeBinary(t *testing.T) {
	repoRoot := t.TempDir()
	...
	expected := hawpBinaryPath(repoRoot)
	if !strings.Contains(content, `"command": "`+expected+`"`) {
		t.Errorf("cursor MCP config missing native binary path %q, got:\n%s", expected, content)
	}
}

func TestProviderNativeCommandsCarryRepoRoot(t *testing.T) {
	root := t.TempDir()
	want := []string{"mcp", "--repo-root", root}
	for _, entry := range []map[string]any{claudeServerEntry(root), cursorServerEntry(root)} {
		if !reflect.DeepEqual(entry["args"], want) {
			t.Fatalf("missing explicit root: %+v", entry)
		}
		name := "hawp"
		if runtime.GOOS == "windows" {
			name += ".exe"
		}
		if entry["command"] != filepath.Join(root, ".hawp", "bin", name) {
			t.Fatalf("not native: %+v", entry)
		}
	}
	if !strings.Contains(codexTOMLBlock(root), `args = ["mcp", "--repo-root",`) {
		t.Fatal("Codex lacks explicit root")
	}
}
