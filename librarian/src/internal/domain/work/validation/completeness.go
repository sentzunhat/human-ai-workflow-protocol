...
// are skipped.
func CheckClosedTaskCompleteness(workDir string, source Source) ClosedTaskCheck {
	result := ClosedTaskCheck{Status: StatusPass}

	for _, entry := range collectClosedFilesGeneric(filepath.Join(workDir, "closed"), source) {
		...
		content, err := source.ReadFile(entry.filePath)
		if err != nil {
			warnf(source, "skipping unreadable closed plan %s: %v", entry.filePath, err)
			continue
		}
...
func collectClosedFilesGeneric(closedDir string, source Source) []closedFileEntry {
