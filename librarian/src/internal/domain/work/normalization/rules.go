...
// Source supplies the read-only repository operations needed by rule evaluation.
type Source struct {
...
var (
	claimPrefixRe    = regexp.MustCompile(`^\s*-\s*\[[ xX]\]\s*`)
	pathDateRe       = regexp.MustCompile(`/closed/(\d{4})/(\d{2})/(\d{2})/`)
...
func isCanonicalID(value string) bool {
	return canonicalIDRe.MatchString(value) || numericRowIDRe.MatchString(value) || identity.IsFullUUID(value) || identity.ExtractShortUUID(value) != ""
}
...
func HasUnprovenChecklistMarker(content string) bool { return hasUnprovenChecklistMarker(content) }
func HasHeadingNamed(content, heading string) bool   { return hasHeadingNamed(content, heading) }
func AmbiguousClaims(content string) []string        { return AmbiguousVerificationClaims(content) }
...
func EvaluateRules(repoRoot, workRoot, backlogPath string, backlog *NormalizeBacklog, scan *PlanScan, source Source) []FixOperation {
	e := &ruleEvaluator{
