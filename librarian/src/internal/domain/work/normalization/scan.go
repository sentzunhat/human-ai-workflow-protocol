...
// ScanSource supplies read-only filesystem observations for normalization
// scanning. Concrete OS access belongs to application/infrastructure code.
...
func ParseNormalizeBacklog(backlogPath string, source ScanSource) (*NormalizeBacklog, error) {
	content, err := source.ReadFile(backlogPath)
	if err != nil {
		return nil, err
	}
	...
	for index, line := range lines {
		...
		for _, value := range candidates {
			if identity.ExtractIDFromFilename(value) != "" {
				rawID = value
				break
			}
		}
		if rawID == "" {
			for _, value := range candidates {
				if identity.ExtractShortUUID(value) != "" {
					rawID = value
					break
				}
			}
		...
		id := identity.ExtractIDFromFilename(rawID)
		if id == "" {
			id = identity.ExtractShortUUID(rawID)
		}
		if id == "" {
			id = rawID
		}
...
func ReadBacklogID(content string) string {
	...
	if m := uuidFieldRe.FindStringSubmatch(content); m != nil {
		if id := identity.ExtractIDFromFilename(m[1]); id != "" {
			return id
		}
		if id := identity.ExtractShortUUID(m[1]); id != "" {
			return id
		}
	}
	return ""
...
func WalkPlanMarkdown(dir string, source ScanSource) []string {
	var out []string
	...
	for _, entry := range entries {
		...
		if entry.IsDir() {
			out = append(out, WalkPlanMarkdown(full, source)...)
			continue
		}
...
func ScanPlanFiles(workRoot string, source ScanSource) *PlanScan {
	...
	for _, name := range []string{"active", "parked", "closed"} {
		dir := dirs[name]
		_, err := source.Stat(dir)
		scan.DirectoryPresence[name] = err == nil
		paths = append(paths, WalkPlanMarkdown(dir, source)...)
	}
	...
	for _, path := range paths {
		...
		if id == "" {
			...
			id = identity.ExtractIDFromFilename(base)
			if id == "" {
				id = identity.ExtractShortUUID(strings.TrimSuffix(base, ".md"))
			}
		}
		if id == "" {
			continue
