...
func (w *WorkSource) ApplyCompletedActiveRowCleanup(repoRoot string) (ApplyResult, error) {
	...
	for _, row := range backlog.Rows {
		...
		closedResolved, err := filepath.EvalSymlinks(closedRoot)
		if err != nil {
			continue
		}
	...
	backlogTarget, err := filepath.EvalSymlinks(backlogPath)
	if err != nil {
		return result, err
	}
	workTarget, err := filepath.EvalSymlinks(workRoot)
	if err != nil {
		return result, err
	}
	...
	if fi, err := os.Lstat(backlogPath); err != nil {
		return result, err
