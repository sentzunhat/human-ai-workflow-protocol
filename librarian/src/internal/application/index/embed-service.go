...
// NewEmbedServiceWithFactory injects model construction at the composition point,
// keeping the application service independent of infrastructure packages.
...
func (s *EmbedService) Execute(ctx context.Context, backend, modelID string) (EmbedResult, error) {
	...
	if existing, ok, metaErr := db.GetEmbeddingMetadata(); metaErr == nil && ok {
	...
	for i := 0; i < len(chunks); i += batchSize {
		...
		for k, vec := range batchVectors {
			...
			if err := db.UpdateChunkEmbedding(chunks[chunkIdx].ID, vectorJSON); err != nil {
				db.Rollback()
	...
	if totalEmbedded > 0 {
		if metaErr := db.SetEmbeddingMetadata(sqlite.EmbeddingMetadata{
			Backend: backend,
			Model:   modelID,
			Dim:     vectorDims,
		}); metaErr != nil {
