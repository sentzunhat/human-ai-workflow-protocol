...
func NewItem(workDir, itemType, title, inputText string) (*NewItemResult, error) {
	...
	updatedBacklog, err := domainwork.InsertNewItem(string(backlogBytes), item, date)
	if err != nil {
	...
	if fi, err := os.Lstat(planPath); err == nil {
		if !fi.Mode().IsRegular() {
			return nil, fmt.Errorf("plan path %s exists but is not a regular file; refusing to overwrite", planPath)
		}
