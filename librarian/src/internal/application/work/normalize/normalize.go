...
func Normalize(out, errOut io.Writer, opts NormalizeOptions) int {
	...
	if opts.Apply {
		...
		if opts.MigrateFolders {
			...
			closed, err := defaultWorkSource.ApplyClosedRecordNormalization(opts.RepoRoot)
			if err != nil {
				fmt.Fprintf(errOut, "Script error: %v\n", err)
				return 1
			}
	...
	scanSource := normalization.ScanSource{ReadFile: defaultWorkSource.ReadFile, ReadDir: defaultWorkSource.ReadDir, Stat: defaultWorkSource.Stat}
	backlog, err := normalization.ParseNormalizeBacklog(backlogPath, scanSource)
	if err != nil {
		fmt.Fprintf(errOut, "Script error: %v\n", err)
		return 1
	}
	scan := normalization.ScanPlanFiles(workRoot, scanSource)
	...
	report.ResearchQueue = domainwork.BuildResearchQueue(opts.RepoRoot, defaultWorkSource)
