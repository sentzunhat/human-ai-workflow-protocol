...
import (
	...
	workvalidation "github.com/sentzunhat/hawp/librarian/src/internal/domain/work/validation"
...
var validationSource = workvalidation.Source{
	Exists:         infraRepo.Exists,
	ReadDir:        os.ReadDir,
...
func Validate(workDir string) (*domainwork.Report, error) {
	backlog, err := reposwork.ReadBacklog(filepath.Join(workDir, "BACKLOG.md"))
	if err != nil {
		return nil, fmt.Errorf("could not parse BACKLOG.md: %w", err)
	}

	closedFiles := workvalidation.CollectClosedPlanFiles(filepath.Join(workDir, "closed"), validationSource)

	report := &domainwork.Report{
		Backlog:      workvalidation.CheckBacklogConsistency(workDir, backlog, validationSource),
		Completeness: workvalidation.CheckClosedTaskCompleteness(workDir, validationSource),
		Evidence:     workvalidation.CheckEvidenceIntegrity(workDir, closedFiles, validationSource),
		Clarity:      workvalidation.CheckVerificationClarity(closedFiles, validationSource),
		DeadLinks:    workvalidation.CheckDeadLinks(workDir, validationSource),
	}
	report.Summarize()
	return report, nil
}
...
func Render(out io.Writer, workDir string, r *domainwork.Report) int {
	...
	for _, id := range r.Backlog.NonCanonicalActiveItems {
		line("  ✗ active row uses non-canonical new-work ID: %s", id)
	}
	line("")
