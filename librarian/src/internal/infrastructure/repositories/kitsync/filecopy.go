...
func (f *FileCopier) ReadDir(dir string) ([]os.DirEntry, error) { return os.ReadDir(dir) }

// Stat calls os.Stat.
func (f *FileCopier) Stat(path string) (os.FileInfo, error) { return os.Stat(path) }
...
// Open calls os.Open.
func (f *FileCopier) Open(path string) (*os.File, error) { return os.Open(path) }

// CreateTemp creates a temporary file in dir with the given pattern.
func (f *FileCopier) CreateTemp(dir, pattern string) (*os.File, string, error) {
