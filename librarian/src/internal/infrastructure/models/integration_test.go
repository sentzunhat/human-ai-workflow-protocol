...
func TestIntegration_ONNX_LLM_Status(t *testing.T) {
	...
	t.Logf("SupportedModels count: %d", len(onnxmodel.SupportedLLMModels))
	for model := range onnxmodel.SupportedLLMModels {
		t.Logf("  - %s", model)
	}

	_, err := onnxmodel.NewONNXLLMClient(onnxmodel.DefaultLLMModel)
	if err == nil {
...
func TestIntegration_Summary(t *testing.T) {
	...
	results["ONNX LLM"] = fmt.Sprintf("⚠️  needs -tags ORT build (verified working manually; %d model(s) registered)", len(onnxmodel.SupportedLLMModels))
