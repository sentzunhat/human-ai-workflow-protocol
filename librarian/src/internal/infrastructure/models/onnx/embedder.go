...
// EmbedModelInfo holds metadata for supported ONNX embedding models.
...
// SupportedEmbedModels lists all available ONNX embedding models.
var SupportedEmbedModels = map[string]EmbedModelInfo{
	"bge-base-en-v1.5": {
...
// DefaultEmbedModel is the recommended ONNX embedding model (best quality).
const DefaultEmbedModel = "bge-base-en-v1.5"

// pipelineCounter ensures unique pipeline names across embedder and LLM instances.
...
func NewONNXEmbedder(model string) (*ONNXEmbedder, error) {
	...
	info, ok := SupportedEmbedModels[model]
	if !ok {
		return nil, fmt.Errorf("unsupported ONNX model: %s; supported: %v", model, embedSupportedModelNames())
	}
...
func (e *ONNXEmbedder) Backend() string { return "onnx" }
...
// GetEmbedModelPath returns the path where an embedding model should be downloaded to.
// Path: ~/.hawp/models/embedding/{model-name}/
func GetEmbedModelPath(home, model string) string {
	return filepath.Join(home, ".hawp", "models", "embedding", model)
}

func embedSupportedModelNames() []string {
	names := make([]string, 0, len(SupportedEmbedModels))
	for name := range SupportedEmbedModels {
