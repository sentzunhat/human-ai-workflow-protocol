...
func BenchmarkAll_ONNX_Embeddings_MiniLM(b *testing.B) {
	embedder, err := onnxmodel.NewONNXEmbedder("all-MiniLM-L6-v2")
	if err != nil {
...
func BenchmarkAll_ONNX_Embeddings_BGE(b *testing.B) {
	embedder, err := onnxmodel.NewONNXEmbedder("bge-base-en-v1.5")
	if err != nil {
...
func BenchmarkAll_Ollama_Embeddings_MiniLM(b *testing.B) {
	embedder, err := ollamamodel.NewOllamaEmbedder("http://localhost:11434", "all-minilm")
	if err != nil {
...
func BenchmarkAll_Ollama_Embeddings_Nomic(b *testing.B) {
	embedder, err := ollamamodel.NewOllamaEmbedder("http://localhost:11434", "nomic-embed-text")
	if err != nil {
...
func TestBenchmarkAll_ONNX_LLM_Status(t *testing.T) {
	...
	t.Logf("ONNX LLM: SupportedModels count = %d", len(onnxmodel.SupportedLLMModels))
...
func BenchmarkAll_Ollama_LLM_Qwen(b *testing.B) {
	client, err := ollamamodel.NewOllamaLLMClient("http://localhost:11434", "qwen3.5:4b")
	if err != nil {
...
func BenchmarkAll_Ollama_LLM_Mistral(b *testing.B) {
	client, err := ollamamodel.NewOllamaLLMClient("http://localhost:11434", "mistral")
	if err != nil {
...
func TestBenchmarkAll_Full_Pipeline_ONNX(t *testing.T) {
	...
	emb, err := onnxmodel.NewONNXEmbedder("all-MiniLM-L6-v2")
	if err != nil {
		t.Fatalf("ONNX embedder: %v", err)
	}
	defer emb.Close()

	llmClient, err := ollamamodel.NewOllamaLLMClient("http://localhost:11434", "qwen3.5:4b")
	if err != nil {
...
func TestBenchmarkAll_Full_Pipeline_Ollama(t *testing.T) {
	...
	emb, err := ollamamodel.NewOllamaEmbedder("http://localhost:11434", "all-minilm")
	if err != nil {
...
func TestBenchmarkAll_SummaryReport(t *testing.T) {
	...
	if emb, err := onnxmodel.NewONNXEmbedder("all-MiniLM-L6-v2"); err == nil {
		start := time.Now()
	...
	if emb, err := onnxmodel.NewONNXEmbedder("bge-base-en-v1.5"); err == nil {
		start := time.Now()
		vec, _ := emb.Embed(ctx, "test")
		elapsed := time.Since(start)
	...
	if emb, err := ollamamodel.NewOllamaEmbedder("http://localhost:11434", "all-minilm"); err == nil {
		start := time.Now()
		vec, _ := emb.Embed(ctx, "test")
		elapsed := time.Since(start)
	...
	if emb, err := ollamamodel.NewOllamaEmbedder("http://localhost:11434", "nomic-embed-text"); err == nil {
		start := time.Now()
		vec, _ := emb.Embed(ctx, "test")
		elapsed := time.Since(start)
	...
	results["ONNX LLM"] = map[string]interface{}{
		"status": fmt.Sprintf("needs -tags ORT build (verified working manually; %d model(s) registered)", len(onnxmodel.SupportedLLMModels)),
	}

	// Ollama LLM
	results["Ollama LLM"] = make(map[string]interface{})
	if client, err := ollamamodel.NewOllamaLLMClient("http://localhost:11434", "qwen3.5:4b"); err == nil {
	...
	if client, err := ollamamodel.NewOllamaLLMClient("http://localhost:11434", "mistral"); err == nil {
