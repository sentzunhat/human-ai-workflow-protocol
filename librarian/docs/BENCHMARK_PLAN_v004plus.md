...
Create `librarian/go/benchmarks/` directory with:

```
librarian/go/benchmarks/
...
# Saves results to: librarian/go/benchmarks/results/YYYY-MM-DD-suite.json
```
...
go test -v ./benchmarks/... \
    ...
    -baseline librarian/go/benchmarks/results/2026-07-26-v003-baseline.json
```
...
- `librarian/go/README.md` — quick performance reference
...
jobs:
  benchmark:
    ...
    steps:
      ...
      - name: Commit Results
        run: |
          git add librarian/go/benchmarks/results/
